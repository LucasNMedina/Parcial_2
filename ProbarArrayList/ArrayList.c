...
{
    int returnAux=-1;
    if(pList!=NULL && pElement!=NULL && (index>=0 && index<=pList->size) && resizeUp(pList)==0){
        if(index==pList->size){
...
ArrayList* al_clone(ArrayList* pList)
{
    ...
    if(pList!=NULL){
        ...
        for(i=0; i<pList->size; i++){
            pList->add(returnAux,al_get(pList,i));
        }
        resizeDown(returnAux);
    }
    return returnAux;
}
            ...
            for(j=i+1; j<pList->size; j++){
                if(order){
                    if(pFunc(pList->pElements[i],pList->pElements[j])== 1){
                        aux=pList->pElements[j];
                        pList->pElements[j]=pList->pElements[i];
                        pList->pElements[i]=aux;
                    }
                }
                else{
                    if(pFunc(pList->pElements[i],pList->pElements[j])== -1){
...
int resizeDown(ArrayList* pList)
{
    ...
    if(pList != NULL){
        ...
        if((pList->reservedSize-pList->size)> AL_INCREMENT){
            pAux=(void**)realloc(pList->pElements, sizeof(void*)*(pList->size+AL_INCREMENT));
            if(pAux != NULL){
                pList->pElements=pAux;
                pList->reservedSize=pList->size+AL_INCREMENT;
            }
